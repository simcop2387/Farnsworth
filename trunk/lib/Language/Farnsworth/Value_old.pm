...
sub add
{
  ...
  my $tv = ref($two) && $two->isa("Language::Farnsworth::Value") ? $two->{pari} : $two;
  ...
  if ($one->{dimen}{dimen}{string})
  {
  	...
  	$new = new Language::Farnsworth::Value($one->{pari} . $tv, $one->{dimen});
  }
  ...
  elsif (($one->{dimen}{dimen}{date}) && ($two->{dimen}->compare({dimen=>{time => 1}}))) #check if we are adding time to a date
  {
		  ...
		  $new = new Language::Farnsworth::Value($delta, {date=>1});
  }
  else
  {
	$new = new Language::Farnsworth::Value($one->{pari} + $tv, $one->{dimen});
...
sub subtract
{
  ...
  if (!$rev)
  {
	  if (($one->{dimen}{dimen}{date}) && ($two->{dimen}{dimen}{date}))
	  {
		  ...
		  my $seconds = new Language::Farnsworth::Value(Delta_Format($delta,1,"%st"), {time => 1});
	  ...
	  {
		  $new = new Language::Farnsworth::Value($one->{pari} - $tv, $one->{dimen}); #if !$rev they are in order
	  }
  }
  else
  {
	  die "some mistake happened here in subtraction\n"; #to test later on
#      $new = new Language::Farnsworth::Value($tv - $one->{pari}, $one->{dimen}); #if !$rev they are in order
  }
  return $new;
}
...
sub mult
{
  ...
  #check for $two being a simple value
  my $tv = ref($two) && $two->isa("Language::Farnsworth::Value") ? $two->{pari} : $two;
  my $td = ref($two) && $two->isa("Language::Farnsworth::Value") ? $two->{dimen} : new Language::Farnsworth::Dimension();
  ...
  #moving this down so that i don't do any math i don't have to
  my $new = new Language::Farnsworth::Value($one->{pari} * $tv, $nd);
  return $new;
}
...
sub div
{
  ...
  if (!$rev)
  {
	  $new = new Language::Farnsworth::Value($one->{pari} / $tv, $nd); #if !$rev they are in order
  }
  else
  {
      $new = new Language::Farnsworth::Value($tv / $one->{pari}, $nd); #if !$rev they are in order
  }
...
sub pow
{
  ...
  #check for $two being a simple value
  my $tv = ref($two) && $two->isa("Language::Farnsworth::Value") ? $two->{pari} : $two;
  my $td = ref($two) && $two->isa("Language::Farnsworth::Value") ? $two->{dimen} : undef;
  ...
  if (!$rev)
  {
	  $new = new Language::Farnsworth::Value($one->{pari} ** $tv, $one->{dimen}->mult($tv)); #if !$rev they are in order
  }
  else
  {
#	  print Dumper(\@_);
	  #print "POW: $tv :: $two :: $one\n";
      $new = new Language::Farnsworth::Value($tv ** $one->{pari}, $one->{dimen}->mult($tv)); #if !$rev they are in order
  }
